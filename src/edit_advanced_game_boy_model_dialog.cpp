...
EditAdvancedGameBoyModelDialog::EditAdvancedGameBoyModelDialog(GameWindow *window) : window(window) {
    ...
    auto add_tab = [&tab_widget, &dialog] (
            ...
            QCheckBox **allow_external_boot_rom,
            bool allow_external_boot_rom_v,
        ...
        auto *boot_rom_label = new QLabel("External boot ROM path:", boot_rom_widget);
        ...
        *boot_rom_path = new QLineEdit(boot_rom_widget);
        ...
        auto *use_custom_label = new QLabel("Use external boot ROM:", use_custom_w);
        use_custom_label->setMinimumWidth(label_width);
        use_custom_l->addWidget(use_custom_label);
        *allow_external_boot_rom = new QCheckBox(use_custom_w);
        (*allow_external_boot_rom)->setChecked(allow_external_boot_rom_v);
        use_custom_l->addWidget(*allow_external_boot_rom);
    ...
    add_tab("Game Boy Color", &this->gbc_boot_rom_le, this->window->gbc_boot_rom_path, &this->gbc_rev, {
            ...
            }, this->window->gbc_rev, &EditAdvancedGameBoyModelDialog::find_gbc_boot_rom, &this->gbc_allow_external_boot_rom, window->gbc_allow_external_boot_rom, &this->gbc_border_cb, window->gbc_border, {
    ...
    add_tab("Game Boy Advance", &this->gba_boot_rom_le, this->window->gba_boot_rom_path, &this->gba_rev, {
            ...
            }, this->window->gba_rev, &EditAdvancedGameBoyModelDialog::find_gba_boot_rom, &this->gba_allow_external_boot_rom, window->gba_allow_external_boot_rom, &this->gba_border_cb, window->gba_border);
    ...
    add_tab("Super Game Boy", &this->sgb_boot_rom_le, this->window->sgb_boot_rom_path, &this->sgb_rev, {
            ...
            }, this->window->sgb_rev, &EditAdvancedGameBoyModelDialog::find_sgb_boot_rom, &this->sgb_allow_external_boot_rom, window->sgb_allow_external_boot_rom, &this->sgb_border_cb, window->sgb_border, {
    ...
    add_tab("Super Game Boy 2", &this->sgb2_boot_rom_le, this->window->sgb2_boot_rom_path, &this->sgb2_rev, {
            ...
            }, this->window->sgb2_rev, &EditAdvancedGameBoyModelDialog::find_sgb2_boot_rom, &this->sgb2_allow_external_boot_rom, window->sgb2_allow_external_boot_rom, &this->sgb2_border_cb, window->sgb2_border, {
...
void EditAdvancedGameBoyModelDialog::perform_accept() {
    ...
    if(this->window->instance->is_rom_loaded()) {
        switch(this->window->gb_type) {
            case GameWindow::GameBoyType::GameBoyGB:
                requires_reset = (current_boot_rom != this->gb_boot_rom_le->text().toStdString()) || (current_revision != this->gb_rev->currentData().toInt()) || (this->gb_allow_external_boot_rom->isChecked() != this->window->gb_allow_external_boot_rom);
                break;
            case GameWindow::GameBoyType::GameBoyGBC:
                ...
                ) || (this->gbc_allow_external_boot_rom->isChecked() != this->window->gbc_allow_external_boot_rom);
                break;
            case GameWindow::GameBoyType::GameBoyGBA:
                requires_reset = (current_boot_rom != this->gba_boot_rom_le->text().toStdString()) || (current_revision != this->gba_rev->currentData().toInt()) || (this->gba_allow_external_boot_rom->isChecked() != this->window->gba_allow_external_boot_rom);
                break;
            case GameWindow::GameBoyType::GameBoySGB:
                requires_reset = (current_boot_rom != this->sgb_boot_rom_le->text().toStdString()) || (current_revision != this->sgb_rev->currentData().toInt()) || (this->sgb_allow_external_boot_rom->isChecked() != this->window->sgb_allow_external_boot_rom);
                break;
            case GameWindow::GameBoyType::GameBoySGB2:
                requires_reset = (current_boot_rom != this->sgb2_boot_rom_le->text().toStdString()) || (current_revision != this->sgb2_rev->currentData().toInt()) || (this->sgb2_allow_external_boot_rom->isChecked() != this->window->sgb2_allow_external_boot_rom);
                break;
            default:
    ...
    this->window->gb_allow_external_boot_rom = this->gb_allow_external_boot_rom->isChecked();
    this->window->gbc_allow_external_boot_rom = this->gbc_allow_external_boot_rom->isChecked();
    this->window->gba_allow_external_boot_rom = this->gba_allow_external_boot_rom->isChecked();
    this->window->sgb_allow_external_boot_rom = this->sgb_allow_external_boot_rom->isChecked();
    this->window->sgb2_allow_external_boot_rom = this->sgb2_allow_external_boot_rom->isChecked();
